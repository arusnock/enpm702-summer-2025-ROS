...
public:
    RandomController() : Node("random_controller"), gen_(rd_())
    {
        ...
        declare_parameter_with_description("linear_min", 0.1, "Minimum linear velocity");
        declare_parameter_with_description("linear_max", 0.2, "Maximum linear velocity");
        declare_parameter_with_description("angular_min", -1.0, "Minimum angular velocity");
        declare_parameter_with_description("angular_max", 1.0, "Maximum angular velocity");
        declare_parameter_with_description("timer_period_ms", 200, "Timer period in milliseconds");
        ...
        param_callback_handle_ = this->add_on_set_parameters_callback(
            std::bind(&RandomController::parameter_callback, this, std::placeholders::_1));

        RCLCPP_INFO(this->get_logger(), "Random Controller Node Started");
        RCLCPP_INFO(this->get_logger(), "Publishing random velocities to /cmd_vel every %d ms", timer_period_ms_);
        RCLCPP_INFO(this->get_logger(), "Linear velocity range: [%.2f, %.2f] m/s", linear_min_, linear_max_);
        RCLCPP_INFO(this->get_logger(), "Angular velocity range: [%.2f, %.2f] rad/s", angular_min_, angular_max_);
...
private:
    ...
    void declare_parameter_with_description(const std::string& name, int default_value, const std::string& description)
    {
        rcl_interfaces::msg::ParameterDescriptor desc;
        desc.description = description;
        this->declare_parameter(name, default_value, desc);
    ...
    {
        linear_dist_ = std::uniform_real_distribution<double>(linear_min_, linear_max_);
        angular_dist_ = std::uniform_real_distribution<double>(angular_min_, angular_max_);
    }
        ...
        const std::vector<rclcpp::Parameter>& parameters)
    {
        ...
        for (const auto& param : parameters) {
            if (param.get_name() == "linear_min" || param.get_name() == "linear_max" ||
                param.get_name() == "angular_min" || param.get_name() == "angular_max") {
                ...
                if (param.get_type() != rclcpp::ParameterType::PARAMETER_INTEGER) {
                    result.successful = false;
                    result.reason = "Parameter " + param.get_name() + " must be of type integer";
                    return result;
                }
