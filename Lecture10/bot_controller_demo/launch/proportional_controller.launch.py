...
def generate_launch_description():
    ...
    kp_linear_arg = DeclareLaunchArgument(
        "kp_linear", default_value="0.5", description="Linear proportional gain"
    )

    kp_angular_arg = DeclareLaunchArgument(
        "kp_angular", default_value="1.0", description="Angular proportional gain"
    )

    goal_x_arg = DeclareLaunchArgument(
        "goal_x", default_value="2.0", description="Goal x-coordinate"
    )

    goal_y_arg = DeclareLaunchArgument(
        "goal_y", default_value="1.0", description="Goal y-coordinate"
    )
    ...
    linear_tolerance_arg = DeclareLaunchArgument(
        "linear_tolerance", default_value="0.1", description="Linear position tolerance"
    )
